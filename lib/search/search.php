<?php
	function getSearchQueryString($terms) {
		$lib_search = new Libs_Search($terms);
		return $lib_search->GetSearchQueryString();
	}
			...
			if(is_array($wordsArray) && count($wordsArray) > 0)
			{
				...
				foreach($wordsArray as $word)
				{
					...
					if($word == self::SEARCH_WORD_NOT)
					{
						...
						/*if(($keywordCount > 0) && 
											(
											!strstr($keywordsArray[$keywordCount - 1], self::SEARCH_WORD_NOT_OPERATOR) || 
											(strpos($keywordsArray[$keywordCount - 1], self::SEARCH_WORD_NOT_OPERATOR) > 0)
											)
						  )
						{
							$keywordsArray[$keywordCount - 1] = self::SEARCH_WORD_NOT_OPERATOR . 
																	$keywordsArray[$keywordCount - 1];
						}*/
